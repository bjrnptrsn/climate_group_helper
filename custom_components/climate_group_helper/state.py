...
class FilterState(TargetState):
    """Zustand, der ausschließlich als Filter für Maskierungen dient."""
    
    @classmethod
    def from_keys(cls, attributes: list[str]) -> FilterState:
        """Erstellt einen FilterState mit Platzhaltern für die angegebenen Attribute."""
        placeholders = {}
        for attr in attributes:
            if attr in TargetState.__annotations__:
                # Leerer String als Platzhalter ( != None )
                placeholders[attr] = True
        return cls(**placeholders)
    ...
    def __repr__(self) -> str:
        ...
        d = asdict(self)
        # Filter None and ""
        filtered = {k: v for k, v in d.items() if v is not None and v != ""}
        # Format
        attrs = ", ".join(f"{k}={repr(v)}" for k, v in filtered.items())
...
class ChangeState(TargetState):
    ...
    @classmethod
    def create_diff(cls, event: Event, target_state: TargetState) -> ChangeState:
        """
        Calculates the difference between the Event's new state and the TargetState.
        Returns a ChangeState containing ONLY the attributes that differ.
        Unchanged or unrelated attributes remain None.
        """
